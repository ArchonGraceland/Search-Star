...
> **Post-Pass-1 update (2026-04-24):** F35 resolved out-of-band before Pass 2 opened. `src/app/api/admin/create-test-users/route.ts` deleted; production Supabase verified to contain zero `testuser.*@test.searchstar.dev` auth rows (route was never actually invoked against prod). The F35 entry below is preserved as-written for Pass 1 historical fidelity — the disposition has been taken as option (a), delete outright. All other findings still stand as Pass 2 / Pass 3 work.
