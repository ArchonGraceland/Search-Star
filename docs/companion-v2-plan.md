...
|---|---|---|---|
| 10A | Foundations: doc cleanup, cross-commitment memory | Open | Self-pilot accumulating one completed commitment to test against (no current blocker; can ship the schema and the read path now and exercise once a commitment completes) |
| 10B | Conversation-aware participation (architectural rewrite of the trigger model) | Open | 10A landed; one week of self-pilot under the addressee fix to confirm it isn't masking unrelated issues |
...
includes wiring it). The summary itself is generated by a small
Companion call with a dedicated prompt — distinct from the existing
`DAY90_SUMMARY_SYSTEM_PROMPT` in `src/lib/anthropic.ts`, which is
written for sponsors about to release payment. The completion summary
is for the Companion's own future memory: a few sentences naming what
the practice was, what shape the 90 days took, where the work lived
strongest, what threads were left open.
